...
class Database:
    ...
    async def _seed_reference_data(self) -> None:
        ...
        await pool.execute(
            ...
            VALUES ('12345', 'Anusha', '+91 9876543210')
            ON CONFLICT (customer_id) DO NOTHING;
            INSERT INTO appointments (id, customer_id, date, time, status)
            VALUES ('appointment-12345', '12345', '2026-08-28', '15:00', 'scheduled')
            ON CONFLICT (id) DO NOTHING;
            INSERT INTO consultation_slots (date, time, available)
            SELECT * FROM (VALUES
                ('2026-08-28', '10:00', TRUE),
                ('2026-08-28', '15:00', TRUE),
                ('2026-08-29', '11:00', TRUE)
            ) AS slots(date, time, available)
            WHERE NOT EXISTS (SELECT 1 FROM consultation_slots);
    ...
    async def get_messages(self, session_id: str) -> list[dict[str, Any]]:
        ...
        rows = await pool.fetch(
            "SELECT id, role, content FROM messages WHERE session_id = $1 ORDER BY timestamp, id",
            session_id,
        )
        return [
            ...
            async with connection.transaction():
                for message in messages:
                    ...
                    await connection.execute(
                        "INSERT INTO messages (id, session_id, role, content) VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO NOTHING",
                        message_id,
                        session_id,
                        message["role"],
